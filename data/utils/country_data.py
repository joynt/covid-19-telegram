...
def parse_countries(default_countries: list) -> list:
...
def compute_countries_confirmed_cases(default_countries) -> dict:

    # Get all data for each country
    all_countries = parse_countries(default_countries)
    ...
    for c in all_countries:
        ...
        for idx, d in enumerate(local_data):
            if 1 < idx < len(local_data) and d - local_data[idx - 1] == 0:
